//! Poll live agent transcript files and emit a frontend wake-up event when
//! the file actually advances. Agent hook status changes are useful timing
//! hints, but remote-control writes can land in the transcript slightly after
//! `needs_input` / `completed`; this watcher follows the JSONL file itself.
