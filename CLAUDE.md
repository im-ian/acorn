...
- **Agent conversation tracking via PATH shim.** Every Acorn session exports `ACORN_AGENT_STATE_DIR` pointing at `<data_dir>/agent-state/<session-uuid>/`. Shims (`src-tauri/shims/claude.sh`, `codex.sh`) materialise at app start under `<data_dir>/shims/` and are prepended to every PTY's `PATH`. The shims do **not** auto-resume; auto-resume would silently fold unrelated turns into one ever-growing JSONL and inflate context costs (PR #132's original design). Instead each shim captures the new transcript's UUID after a fresh bare-flag run, and the app surfaces a focus-time "이전 대화 이어하기" modal driven by `commands::get_claude_resume_candidate`. The `claude` shim snapshots `~/.claude/projects/<slug>/*.jsonl` before/after the run and writes the new UUID to `claude.id`. The `codex` shim does the same for `$CODEX_HOME/sessions/.../rollout-*.jsonl` and writes to `codex.id`; on subsequent zero-arg invocations codex still gets `codex resume <uuid>` because codex has no deterministic id flag (the modal pattern doesn't apply — codex auto-resumes are still the only way to continue a specific rollout from a bare invocation). Explicit `--resume`/`--continue`/`--session-id` from the user is always passthrough. Acknowledgment is tracked in a sibling `claude.id.acknowledged` file — the modal pops only when `claude.id != claude.id.acknowledged`. Bundled via `include_str!` — no extra build step.
