...
- **Agent conversation persistence via PATH shim.** Every Acorn session exports `ACORN_RESUME_TOKEN` (= Acorn session UUID — deliberately reused, NOT a fresh mint, so claude's JSONL at `~/.claude/projects/<slug>/<session-id>.jsonl` is the same id `session_status::detect` looks up; minting a separate UUID would break that lookup and leave the Sidebar dot stuck on the descendant-process fallback). It also exports `ACORN_AGENT_STATE_DIR` pointing at `<data_dir>/agent-state/<session-uuid>/`. Shims (`src-tauri/shims/claude.sh`, `codex.sh`) materialise at app start under `<data_dir>/shims/` and are prepended to every PTY's `PATH`. The `claude` shim injects `--session-id $ACORN_RESUME_TOKEN` (deterministic create-or-resume). The `codex` shim — codex has no deterministic id flag — captures the auto-minted UUID by diffing `$CODEX_HOME/sessions/.../rollout-*.jsonl` before/after the first zero-arg run, stores it in `codex.id`, and execs `codex resume <uuid>` on subsequent zero-arg invocations. Non-default invocations (`codex foo`, `claude --help`) passthrough untouched. All applied uniformly to regular + control sessions regardless of the daemon killswitch. Bundled via `include_str!` — no extra build step.
