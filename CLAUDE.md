...
- **Agent conversation persistence via PATH shim.** Every Acorn session gets a stable resume token minted on first spawn (persisted as `Session.agent_resume_token`) and exported as `ACORN_RESUME_TOKEN`, plus a per-session scratch dir under `<data_dir>/agent-state/<uuid>/` exported as `ACORN_AGENT_STATE_DIR`. Shims (`src-tauri/shims/claude.sh`, `codex.sh`) materialise at app start under `<data_dir>/shims/` and are prepended to every PTY's `PATH`. The `claude` shim injects `--session-id $ACORN_RESUME_TOKEN` (deterministic create-or-resume). The `codex` shim — codex has no deterministic id flag — captures the auto-minted UUID by diffing `$CODEX_HOME/sessions/.../rollout-*.jsonl` before/after the first zero-arg run, stores it in `codex.id`, and execs `codex resume <uuid>` on subsequent zero-arg invocations. Non-default invocations (`codex foo`, `claude --help`) passthrough untouched. All applied uniformly to regular + control sessions regardless of the daemon killswitch. Bundled via `include_str!` — no extra build step.
