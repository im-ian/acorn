...
- **Agent conversation tracking via filesystem-watcher persister.** The shim-based capture (`src-tauri/shims/claude.sh` + `codex.sh`) was structurally fragile: macOS `/etc/zprofile` runs `path_helper` for every login zsh, which rebuilds `PATH` from `/etc/paths` + `/etc/paths.d/*` and pushes Acorn-prepended entries to the back. Any user-rc `PATH` prepend (`export PATH="$HOME/.local/bin:$PATH"`) buried the shim further, real `claude`/`codex` resolved first, the shim never ran, and the focus-time "이전 대화 이어하기" modal was dead-on-arrival for most users. The shim is gone. Replacement: `agent_resume_persister` spawns at boot and polls `transcript_watcher::collect_live_mappings` on a 2-second interval. That helper walks every Acorn session's PTY descendant tree, finds the live `claude` / `codex` / `agy` process, resolves the transcript it's writing (cwd + mtime + start-time match against `~/.claude/projects/<slug>/` for claude, falling back to transcript cwd metadata when Claude's slug differs; `$CODEX_HOME/sessions/<y>/<m>/<d>/` for codex; or `${ANTIGRAVITY_DIR:-~/.gemini}/antigravity*/brain/<uuid>/.system_generated/logs/transcript.jsonl` for Antigravity), and the persister writes the resolved UUID to `<data_dir>/agent-state/<session-uuid>/{claude,codex,antigravity}.id` whenever it differs from the current value. For Antigravity, it also writes `antigravity.cwd` so project-scoped history can still place brain transcripts that do not include cwd/workspace metadata. The frontend modal calls the provider-specific `get_*_resume_candidate` command for Claude, Codex, and Antigravity on session focus; those compare `*.id` against `*.id.acknowledged` (written by the modal's dismiss path) and surface a candidate only when a *new* UUID has landed. Codex no longer auto-resumes — same modal pattern as claude — because the shell-side `exec codex resume <uuid>` trick disappeared with the shim. `ACORN_AGENT_STATE_DIR` is still exported into the PTY env for end-user scripts that wanted to introspect Acorn state; nothing inside the PTY needs it for the modal to work.
