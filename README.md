...
최신 릴리스 DMG를 내려받아 `/Applications/Acorn.app`에 복사하고 quarantine 속성을 제거합니다. Apple Developer ID로 서명·공증되지 않은 ad-hoc 릴리스에서도 Gatekeeper의 첫 실행 차단을 줄이기 위한 설치 경로입니다.
...
- 보이는 세션들에 같은 입력을 동시에 보내는 multi-input 모드
...
- 세션이 worktree 안에 있는지 사이드바에서 바로 표시

### 💻 PTY 터미널
- 네이티브 PTY 기반 셸 세션
- 세션 재오픈 시 스크롤백 자동 복원
- 단축키로 세션·프로젝트 빠른 순회
- URL 클릭 동작, 줄 높이, 마지막 사용자 프롬프트 고정 옵션
...
### 🎯 우측 패널
- **Files** — 프로젝트 파일 트리, 검색, Git 상태, 코드 보기
...
| 사이드바 / 우측 패널 토글 | `⌘B` / `⌘J` |
| Files / Todos / Commits / Staged / PRs 토글 | `⌘⇧E` / `⌘⇧T` / `⌘⇧C` / `⌘⇧S` / `⌘⇧P` |
| multi-input 토글 | `⌘⌥I` |
...
- Appearance에서 언어, UI 배율, 빌트인/custom theme, 배경 이미지(fit/opacity/blur), 터미널 폰트 3-slot fallback 설정
- AI 기능(머지 메시지 등)에 쓸 공급자(`claude` / `codex` / `gemini` / `ollama` / `llm` / 커스텀) 선택
- PR / Actions 자동 새로고침 주기, GitHub avatar 표시 옵션
- "Open in editor" 외부 명령 지정
- Storage 정리 — orphan worktree, 캐시

### 📊 상태 표시줄
- 메모리 사용량 + 프로세스별 Memory Breakdown
- 현재 worktree 브랜치 + 작업 디렉터리 표시 옵션 + GitHub 계정 배지
...
최신 릴리스 DMG를 내려받아 `/Applications`에 복사하고 quarantine 속성을 제거합니다:

```bash
curl -fsSL https://raw.githubusercontent.com/im-ian/acorn/main/scripts/install-macos.sh | bash
```

설치 위치를 바꾸려면 `ACORN_INSTALL_DIR`를 지정하세요:
